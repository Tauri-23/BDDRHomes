...
export default function AdminSettingsFinancings() {
    const {isSidenavOpen} = useOutletContext();
    const [financings, setFinancings] = useState(null);
    ...
    return(
        <div className={`content1-admin ${isSidenavOpen ? 'compressed' : ''}`}>
            <div className="d-flex justify-content-start mar-bottom-1">
                <Link to={'/BDDRAdmin/Settings'} className="d-flex gap3 align-items-center text-l3 color-black1 text-decoration-none cursor-pointer">
                    <Icon.ChevronLeft/>
                    Back
                </Link>                
            </div>
            ...
            <div className="d-flex flex-direction-y gap1">
                {financings?.length > 0 && financings.map(financing => (
                    <div key={financing.id} className="d-flex justify-content-between align-items-center">
                        <div className="d-flex gap3 align-items-center">
                            <img className="icon3" src={`/src/assets/media/icons/${financing.icon}`}/>
                ...
                {!financings && Array.from({length: 10}, (_, index) => index).map(x => (
